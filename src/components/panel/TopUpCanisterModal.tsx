...
export function TopUpCanisterModal({ isOpen, onClose, canisterId, userBalanceRaw, formatTC, onWithdraw }: TopUpCanisterModalProps) {
  ...
  const userBalanceTC = useMemo(() => {
    ...
    try { return formatTC(userBalanceRaw) } catch { return undefined }
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Top up cycles" className="max-w-md">
      <div className="space-y-4">
          ...
          <div className="flex items-center justify-between">
            ...
            {userBalanceTC !== undefined && (
              <div className="text-right">
                <div className="text-xs text-muted-foreground">Your TC balance</div>
        ...
        <div className="space-y-2">
          <div className="text-sm font-medium">Deposit from wallet</div>
        ...
        <div>
          <Button
            ...
            onClick={() => window.open(`https://cycle.express/?to=${canisterId}`, '_blank', 'noopener,noreferrer')}
          >
            <ExternalLink className="h-4 w-4 mr-2" />
            Top up using Cycle Express
          </Button>
        </div>
      </div>
    </Modal>
